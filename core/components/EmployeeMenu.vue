<template>
  ...
  <div v-if="employee" class="relative">
    <button
      ...
      class="relative p-2 rounded-lg text-primary-600 hover:text-primary-700 hover:bg-primary-50 transition-colors focus:outline-none"
    ...
    >
      ...
      <span class="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-primary-600" aria-hidden="true" />
    </button>
    ...
    >
      <div
        v-if="dropdownOpen"
        class="absolute right-0 mt-2 w-60 bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-gray-100 dark:border-slate-800 py-1 z-50 origin-top-right"
        @click.stop
      >
        <div class="px-4 py-3 border-b border-gray-100 dark:border-slate-800">
          <p class="text-[10px] font-bold uppercase tracking-widest text-primary-600 dark:text-primary-400 mb-0.5">Staff</p>
          <p class="text-sm font-semibold text-gray-900 dark:text-white truncate">{{ employee.firstname }} {{ employee.lastname }}</p>
          <p class="text-xs text-gray-500 dark:text-slate-400 truncate">{{ employee.email }}</p>
        </div>

        <NuxtLink
          to="/hub/dashboard"
          class="flex items-center gap-2 px-4 py-2.5 text-sm text-primary-600 dark:text-primary-400 font-medium hover:bg-primary-50 dark:hover:bg-primary-600/10 transition-colors"
...
const auth = useAuth()
const employee = computed(() => auth.user.value)
