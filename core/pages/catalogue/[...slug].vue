...
const category = computed(() => {
  ...
  return cats.find((c: any) => c.id === psId.value) ?? null
})
...
const siteUrl = computed(() => String((_cfg.public as any).psFrontUrl ?? ''))
...
useHead({
  ...
  script: computed(() => category.value ? [{
    ...
    innerHTML: JSON.stringify({
      ...
      url: `${siteUrl.value}/catalogue/${slug.value}`,
      ...
      <div class="bg-gray-50 border-b border-gray-100">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 py-3">
          <nav class="flex items-center gap-2 text-xs text-gray-400">
            ...
            <span>/</span>
            <span class="text-gray-700 font-medium">{{ category?.name ?? t('catalogue.catalogue_breadcrumb') }}</span>
          </nav>
        </div>
      </div>
